...
const DokumenSaya = () => {
  ...
  const tableData = documents.map(doc => {
    const statusInfo = mapStatusToFrontend(doc.status);
  ...
  const handleEdit = (row) => {
    const path = row.jenis === 'BAPB'
      ? `/vendor/edit-dokumen/bapb/${row.id}`
      : `/vendor/edit-dokumen/bapp/${row.id}`;
    navigate(path);
  };
  ...
  const confirmDelete = async () => {
    if (!deleteModal.documentId || !deleteModal.documentType) return;
    ...
    if (result.success) {
      ...
      fetchDocuments();
      setDeleteModal({ isOpen: false, documentId: null, documentType: null, documentNumber: '' });
    } else {
  ...
  const confirmSubmit = async () => {
    if (!submitModal.documentId || !submitModal.documentType) return;
    ...
    if (result.success) {
      ...
      alert(`Gagal mengajukan dokumen: ${result.error}`);
    }
  };
          ...
          <form onSubmit={handleSearch} className="w-full">
            <div className="relative">
              <input
                type="text"
                name="search"
                placeholder="Cari nomor dokumen, nomor kontrak, atau perusahaan..."
              ...
              <button
                type="submit"
                className="absolute right-3 top-1/2 transform -translate-y-1/2 px-3 py-1 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 transition-colors"
              >
                Cari
              </button>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-end">
            ...
            <div className="w-full lg:w-48">
              ...
              >
                ...
                <option value="reviewed">Direview (BAPB)</option>
                <option value="reviewed_pic">Direview PIC (BAPP)</option>
                <option value="approved">Disetujui (BAPB)</option>
                <option value="approved_direksi">Disetujui Direksi (BAPP)</option>
              </select>
            </div>
