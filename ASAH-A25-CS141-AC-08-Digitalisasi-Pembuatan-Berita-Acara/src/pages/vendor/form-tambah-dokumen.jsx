...
import { useDocuments } from '../../../hooks/useDocuments'; // ← 3 level up
import { useAuth } from '../../../contexts/authcontext'; // ← 3 level up
...
const handleSubmit = async (e) => {
  ...
  console.log('📋 Form data to submit:', { type, ...formData });
  ...
  try {
    ...
    // Tapi untuk sekarang, kita test dengan fetch langsung
    const token = localStorage.getItem('token');
    console.log('🔑 Token available:', !!token);
    
    // Format data sesuai dengan yang diharapkan backend
    const backendData = {
      // Field wajib untuk BAPB (sesuai database schema)
      nomor_bapb: formData.noDokumen,
      no_kontrak: formData.nomorSuratPesanan,
      tanggal_kontrak: formData.tanggalDibuat,
      nilai_kontrak: parseFloat(formData.nilaiKontrak) || 0,
      tanggal_pengiriman: formData.tanggalPengiriman || formData.deadline,
      lokasi_pengiriman: formData.projek || 'Belum diisi',
      rincian_barang: formData.deskripsiPekerjaan,
      keterangan: formData.catatanTambahan || '',
      
      // Field untuk BAPP jika diperlukan
      ...(type === 'bapp' && {
        nomor_bapp: formData.noDokumen,
        lokasi_pekerjaan: formData.lokasiPekerjaan || formData.projek,
        rincian_pekerjaan: formData.deskripsiPekerjaan,
        hasil_pemeriksaan: formData.statusPekerjaan || 'Selesai'
      })
    };
    ...
    const endpoint = type === 'bapb' ? '/api/bapb' : '/api/bapp';
    ...
    if (response.ok) {
      ...
      localStorage.setItem('lastCreatedDoc', JSON.stringify({
        id: result.id,
        type: type,
        nomor: formData.noDokumen
      }));
    } else {
      console.error('❌ API Error:', result);
      alert(`Gagal membuat dokumen: ${result.message || 'Unknown error'}`);
    }
  } catch (error) {
    console.error('💥 Network/Server error:', error);
    alert('Koneksi ke server gagal. Periksa koneksi Anda.');
  } finally {
