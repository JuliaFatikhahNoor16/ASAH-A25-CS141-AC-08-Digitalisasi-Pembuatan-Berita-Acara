...
export const bapbService = {
  ...
  submit: (id) => api.put(`/bapb/${id}/submit`),
...
export const bappService = {
  ...
  submit: (id) => api.put(`/bapp/${id}/submit`),
...
export const documentsService = {
  ...
  getCombined: async (params = {}) => {
    const [bapbResponse, bappResponse] = await Promise.all([
      bapbService.getAll(params),
      bappService.getAll(params)
    ]);

    return {
      data: [
        ...bapbResponse.data.data.map(doc => ({ ...doc, jenis: 'BAPB' })),
        ...bappResponse.data.data.map(doc => ({ ...doc, jenis: 'BAPP' }))
      ],
      pagination: bapbResponse.data.pagination // Ambil pagination dari salah satu
    };
  }
};
